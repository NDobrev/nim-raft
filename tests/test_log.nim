...
suite "RaftLog Tests":
  ...
  test "initRaftLog with non-empty entries should initialize RaftLog correctly":
    ...
    var log = initRaftLog(snapshot, entries)

    check log.lastConfigIndex == RaftLogIndex(7)
    check log.prevConfigIndex == RaftLogIndex(0)
    check log.entriesCount == 3

  test "initRaftLog with non-empty entries and multiple config entries should initialize RaftLog correctly":
    var snapshot = RaftSnapshot(index: RaftLogIndex(5), term: RaftNodeTerm(1), config: RaftConfig(), snapshotId: RaftSnapshotId(1))
